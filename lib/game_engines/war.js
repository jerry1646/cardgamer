...
module.exports = {
  packMessage: function(gameVar, players){
    ...
    message.players = {};
    message.players[players[0].uid] = {
      score: players[0].points,
      playable: players[0].active
    };
    message.players[players[1].uid] = {
  ...
  init: function(gameVar, players){
    ...
    io.to(`${players[0].ioConfig.socketId}`).emit('ready', 'hello game on');
    io.to(`${players[1].ioConfig.socketId}`).emit('ready', 'hello game on');
  },
  ...
  draw: function(request, gameVar, players){
    ...
    function checkWin(cards){
      let value1 = cardParser(cards[players[0].uid]);
      let value2 = cardParser(cards[players[1].uid]);
      if (value1 > value2){
        gameVar.roundWinner = players[0].uid;
        players[0].points++;
      } else if (value1 == value2){
        gameVar.roundWinner = "draw";
      } else {
        gameVar.roundWinner = players[1].uid;
        players[1].points++;
      }
      players[0].active = true;
      players[1].active = true;
    ...
    let cPlayer = (senderId == players[0].uid)?players[0]:players[1];
    if (cPlayer.active == true){
      gameVar.round[cPlayer.uid] = drawCard(cPlayer);
      gameVar.roundWinner = "";
  ...
  endGame: function(gameVar, players){
    ...
    if (point1 > point2){
      gameVar.gameWinner = players[0].uid;
    } else if(point1 == point2){
      gameVar.gameWinner = 'draw';
    } else{
      gameVar.gameWinner = players[1].uid;
    }
