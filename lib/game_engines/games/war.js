  ...
  gameEngine:{
    ...
    init: function(gameVar, players){
      ...
      for (let i of [0,1]){
        ...
        io.to(`${players[i].ioConfig.socketId}`).emit('ready', {gameId: gameVar.gameId, players:[players[0].ioConfig.uid,players[1].ioConfig.uid]});
      }
    },
    ...
    endGame: function(gameVar, players){
      const date = new Date;
      const now = `${date.getUTCFullYear()}-${date.getMonth()}-${date.getDate()}`;
